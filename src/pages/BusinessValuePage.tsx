      ...
      {loading ? (
        ...
        <>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 select-none">
            <div className="bg-ink text-white p-4 rounded-xl border border-brand/20 shadow-sm">
              <div className="flex items-center space-x-2 mb-2">
                <DollarSign className="w-5 h-5 text-accent" />
                <span className="text-xs font-bold text-accent uppercase tracking-wider">预算与成本</span>
              </div>
            ...
            <div className="bg-white border border-gray-200 p-4 rounded-xl shadow-sm">
              ...
              <span className="text-2xl font-extrabold text-ink block">{procurementEntries.length}</span>
            ...
            <div className="bg-white border border-gray-200 p-4 rounded-xl shadow-sm">
              ...
              <span className="text-2xl font-extrabold text-ink block">{commercialEntries.length}</span>
          ...
          {budgetEntries.length > 0 && (
            <div className="space-y-2">
              <h3 className="text-sm font-extrabold text-ink border-b border-gray-200 pb-1 flex items-center">
                <DollarSign className="w-4 h-4 text-brand mr-1" />
                <span>预算与成本核算</span>
              </h3>
              {budgetEntries.map((e) => (
                <div key={e.id} className="bg-white border border-gray-200 rounded-lg p-3 hover:shadow-md transition-shadow flex items-center justify-between">
                  <div className="flex-1 min-w-0">
                    <button onClick={() => navigate("/entry/" + e.id)} className="font-bold text-sm text-link hover:underline text-left truncate block">{e.title}</button>
          ...
          {procurementEntries.length > 0 && (
            <div className="space-y-2">
              <h3 className="text-sm font-extrabold text-ink border-b border-gray-200 pb-1 flex items-center">
                <FileText className="w-4 h-4 text-brand mr-1" />
                <span>采购与报价记录</span>
              </h3>
          ...
          {commercialEntries.length > 0 && (
            <div className="space-y-2">
              <h3 className="text-sm font-extrabold text-ink border-b border-gray-200 pb-1 flex items-center">
                <Briefcase className="w-4 h-4 text-brand mr-1" />
                <span>商业化评估</span>
              </h3>
