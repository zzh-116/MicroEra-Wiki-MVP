...
## 11.7 自动部署（Polling Auto Deploy）

> 代码 push 后 NUC 自动感知并部署，延迟 ≤ 2 分钟。

### 原理

systemd timer 每 2 分钟执行 `scripts/auto-deploy.sh`：
1. `git fetch` 检查远程是否有新 commit
2. 无新 commit → 静默退出
3. 有新 commit → `git pull` → `npm install` → `npm run build` → `systemctl restart microera-wiki`
...
# 2. 安装 systemd service 单元
sudo cp scripts/auto-deploy.service /etc/systemd/system/
sudo cp scripts/auto-deploy.timer   /etc/systemd/system/
...
sudo systemctl list-timers | grep auto-deploy
```
...
| 操作 | 命令 |
|------|------|
| 查看 timer 状态 | `systemctl status auto-deploy.timer` |
| 查看部署日志 | `journalctl -u auto-deploy.service -f` |
| 手动触发一次 | `sudo systemctl start auto-deploy.service` |
